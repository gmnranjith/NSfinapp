...
function showLoanForm() {
    const customers = JSON.parse(localStorage.getItem("customers")) || [];
    if (!customers.length) return formArea.innerHTML = "<p class='text-danger'>Add customers first.</p>";
    formArea.innerHTML = `
        <h4>New Loan</h4>
...
function showCollectionForm() {
    ...
    if (!loans.length || !collectors.length) {
        return formArea.innerHTML = "<p class='text-danger'>Add loans and collectors first.</p>";
    }
    formArea.innerHTML = `
        ...
        <form onsubmit="addCollection(event)">
            ...
            <select id="collectionLoanId" class="form-control mb-2" required>
                ${loans.map(l => `<option value="${l.id}">Loan ${l.id} - ₹${l.amount}</option>`).join('')}
            </select>
